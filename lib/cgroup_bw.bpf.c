...
enum scx_cgroup_consts {
	...
	/* normalized period in nsec: 500 msec */
	CBW_NPERIOD			= (500ULL * 1000ULL * 1000ULL),
			  ...
			  struct scx_cgroup_ctx *cgx)
{
	...
	if (base != CBW_RUNTUME_INF) {
		nr_branch_cgs = cgx->nr_taskable_descendents +
				(cgx->has_llcx ? 1 : 0);
			 ...
			 struct scx_cgroup_ctx *cgx, u64 cgrp_id, u64 nuance)
{
	...
	bpf_for(i, 0, cbw_config.nr_llcs) {
		...
		if (scx_cgroup_bw_throttled(cgrp, idx) == -EAGAIN)
			continue;
