...
enum scx_cgroup_consts {
	...
	/* maximum number of re-enqueue operations in one dispatch */
...
static
bool cbw_reenqueue_cgroup(struct scx_cgroup_ctx *cgx, u64 cgrp_id, int level, u64 now)
{
	struct scx_cgroup_llc_ctx *llcx;
...
int scx_cgroup_bw_reenqueue(void)
{
	...
	bpf_for(i, 0, cbw_nr_taskable_cgroups) {
		...
		if (cbw_reenqueue_cgroup(cur_cgx, cur_cgrp_id, cur_level, now))
			reached_max = true;
	}
