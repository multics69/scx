...
bool cbw_replenish_taskable_cgroup(struct scx_cgroup_ctx *cgx, int level, u64 now)
{
	 ...
	 * to its descendants. If the quota is infinite, we don't need to
	 * replenish the cgroup.
	 */
	...
	budget = ((level == 1) ? cgx->nquota_ub : 0) + burst;

	if (period_end && (READ_ONCE(cgx->budget_remaining) > 0))
