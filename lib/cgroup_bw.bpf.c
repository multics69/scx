...
static
struct scx_cgroup_ctx *cbw_alloc_cgroup_ctx(struct cgroup *cgrp)
{
	return bpf_cgrp_storage_get(&cbw_cgrp_map, cgrp, 0,
...
static
void cbw_init_cgroup_ctx(struct cgroup *cgrp __arg_trusted,
			 struct scx_cgroup_ctx *cgx,
			 u64 period_us, u64 quota_us, u64 burst_us)
{
	/*
	...
	if (cgx->nquota_ub == CBW_RUNTUME_INF)
		cgx->budget_remaining = CBW_RUNTUME_INF;
	else
...
int scx_cgroup_bw_reserve(struct cgroup *cgrp __arg_trusted, int llc_id, u64 slice_ns)
{
	...
	cbw_dbg_fn_cgrp("  llc_id: %d -- slice_ns: %llu -- llcx:budget_remaining: %lld",
			llc_id, slice_ns, READ_ONCE(llcx->budget_remaining));
...
int scx_cgroup_bw_consume(struct cgroup *cgrp __arg_trusted, int llc_id, u64 reserved_ns, u64 consumed_ns)
{
	...
	cbw_dbg_fn_cgrp("  llc_id: %d -- reserved_ns: %llu -- consumed_ns: %llu -- llcx:budget_remaining: %lld -- llcx:runtime_total: %lld",
			llc_id, reserved_ns, consumed_ns,
 ...
 * @ctx: the BPF task context from the scheduler.
...
static
int cbw_replenish_cgroup(struct scx_cgroup_ctx *cgx, u64 cgrp_id, int level, u64 now)
{
	...
	if ((level == 1) && (cgx->nquota != CBW_RUNTUME_INF)) {
		 cbw_dbg_fn("cgid:%llu budget_remaining: %llu -- runtime_total_sloppy: %llu",
			    cgrp_id, cgx->budget_remaining, cgx->runtime_total_sloppy);
		/*
...
static
bool cbw_transit_relenish_stat(u64 from, u64 to)
{
...
int replenish_timerfn(void *map, int *key, struct bpf_timer *timer)
{
	 ...
	 */
	if (!cbw_transit_relenish_stat(CBW_REPLENISH_STAT_IDLE,
				       CBW_REPLENISH_STAT_TOP_HALF_RUNNING))
		return 0;

	/*
	 * Reset the runtime_total of each LLC context and update the array of
	 * taskable cgroup IDs (cbw_taskable_cgroup_ids). This prevents the
	 * runtime_total_sloppy at the cgroup level from being mixed with the
	 * runtime_total of the LLC level in a previous period.
	 */
	root_cgrp = bpf_cgroup_from_id(1);
	if (!root_cgrp) {
		cbw_err("Failed to fetch the root cgroup pointer.");
	...
	bpf_for(i, 0, cbw_nr_taskable_cgroups) {
		...
		cbw_replenish_cgroup(cur_cgx, cur_cgrp_id, cur_level, now);
	}
	 ...
	 */
transit_out:
	cbw_transit_relenish_stat(CBW_REPLENISH_STAT_TOP_HALF_RUNNING,
...
int scx_cgroup_bw_reenqueue(void)
{
	 ...
	 */
	if (!cbw_transit_relenish_stat(CBW_REPLENISH_STAT_BOTTOM_HALF_READY,
	 ...
	 */
transit_out:
	cbw_transit_relenish_stat(CBW_REPLENISH_STAT_BOTTOM_HALF_RUNNING,
