			...
			u64 consumed_ns)
{
	 ...
	 *       \== consumed_ns ==/
	 */
	period_duration = time_delta(scx_bpf_now(), cgx->period_start_clk);
...
int scx_cgroup_bw_consume(struct cgroup *cgrp __arg_trusted, int llc_id, u64 consumed_ns)
{
	...
	cbw_dbg_fn_cgrp("  llc_id: %d -- reserved_ns: %llu -- consumed_ns: %llu -- llcx:budget_remaining: %lld -- llcx:runtime_total: %lld",
			llc_id, consumed_ns,
			READ_ONCE(llcx->budget_remaining),
...
static
bool cbw_replenish_cgroup(struct scx_cgroup_ctx *cgx, int level, u64 now)
{
...
int replenish_timerfn(void *map, int *key, struct bpf_timer *timer)
{
	...
	bpf_for(i, 0, cbw_nr_taskable_cgroups) {
		 ...
		 */
		if (cbw_replenish_cgroup(cur_cgx, cur_level, now)) {
