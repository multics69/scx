...
bool cbw_replenish_taskable_cgroup(struct scx_cgroup_ctx *cgx, int level, u64 now)
{
	...
	if (level == 1) {
		 ...
		 * (level == 1) because only the subroot cgroup distributes
		 * the budget to its descendants.
		 *
		...
		if (READ_ONCE(cgx->budget_remaining) > burst)
			WRITE_ONCE(cgx->budget_remaining, burst);

		dbg_cgx(cgx, "replenished-N: ");
	}
