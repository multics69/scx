 ...
 * cgroups without holding an RCU lock. The generation ID increases when
 * a taskable cgroup is created or when it is deleted.
 */
struct taskable_cgroup_ids {
	u64		cgrp_gen_id;
	u64		nr_taskable_cgroups;
	u64		taskable_cgroups[CBW_NR_CGRP_MAX];
};

struct taskable_cgroup_ids	cbw_tcgrp_ids;
u64				cbw_cgrps_gen_id;
