...
struct scx_cgroup_ctx {
	...
	/*
	 * The number of children cgroups.
	 */
	int		nr_children;
...
#define dbg_cgx(cgx, str, ...) do {						\
	cbw_dbg(str "cgid%llu -- cgx:budget_remaining: %lld -- "		\
		...
		"cgx:nquota: %lld -- "						\
		"cgx:nquota_ub: %lld -- "					\
		"cgx:is_throttled: %d -- cgx:nr_children: %d"			\
		...
		cgx->nr_children, cgx->budget_p2c, cgx->budget_c2l);		\
} while (0);
...
static
s64 cbw_calc_budget_tx(struct scx_cgroup_ctx *cgx, s64 tgt_unit, int nr_branch)
{
	s64 ub;
	...
	ub = min(cgx->nquota_ub >> CBW_BUDGET_XFER_MAX_SHIFT,
		 cgx->nquota_ub / (nr_branch << CBW_BUDGET_XFER_MAX_SHIFT));

	if (tgt_unit == 0)
		tgt_unit = ub;

	return clamp(tgt_unit, CBW_BUDGET_XFER_MIN, ub);
}

static
void cbw_update_budget_tx(struct scx_cgroup_ctx *cgx)
{
	cgx->budget_p2c = cbw_calc_budget_tx(cgx, cbw_config.budget_p2c,
					     cgx->nr_children);
	cgx->budget_c2l = cbw_calc_budget_tx(cgx, cbw_config.budget_c2l,
					     cbw_config.nr_llcs);
}
