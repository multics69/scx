					     ...
					     int llc_id)
{
	...
	llcx->btq = (scx_atq_t *)scx_atq_create_size(false, 256);
	if (!llcx->btq) {
...
static
bool cbw_drain_btq_until_throttled(struct scx_cgroup_ctx *cgx,
				   struct scx_cgroup_llc_ctx *llcx)
{
	bool reached_max;
	u64 taskc;
	...
	/*
	 * Check if we reached the CBW_REENQ_MAX_BATCH. This is necessary
	 * to avoid "dispatch buffer overflow".
	 */
	reached_max = (i == CBW_REENQ_MAX_BATCH);
	return reached_max;
}

static
bool cbw_reenqueue_cgroup(struct cgroup *cgrp __arg_trusted,
			  struct scx_cgroup_ctx *cgx,
			  u64 cgrp_id, u64 nuance, u64 now)
{
	struct scx_cgroup_llc_ctx *llcx;
	...
	bpf_for(i, 0, cbw_config.nr_llcs) {
		...
		if (cbw_drain_btq_until_throttled(cgx, llcx))
			reached_max = true;
	}
...
int scx_cgroup_bw_reenqueue(void)
{
	...
	bpf_for(i, 0, cbw_nr_taskable_cgroups) {
		nuance2 = nuance + i;
		idx = nuance2 % cbw_nr_taskable_cgroups;
		ids = MEMBER_VPTR(cbw_taskable_cgroup_ids, [idx]);
		if (!ids) {
			cbw_err("Failed to fetch a taskable cgroup table.");
			continue;
		}
		...
		if (cbw_reenqueue_cgroup(cur_cgrp, cur_cgx, cur_cgrp_id,
					 nuance2, now)) {
			reached_max = true;
		}
