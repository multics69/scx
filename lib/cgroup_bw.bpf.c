...
struct scx_cgroup_ctx {
	...
	/*
	 * Total amount of time executed from the last replenishment
	 * to the current replenishment.
	 */
	s64		runtime_total_at_replenish;
...
struct scx_cgroup_llc_ctx {
	...
	/*
	 * A boolean flag indicating whether there are backlogged tasks or not.
	 *
	 * Note that an unthrottled cgroup can still have backlogged tasks if
	 * they were not requeued within one replenish period.
	 *
	 * Note that scx_atq_nr_queued() is not always allowed because it is
	 * not possible to access the arena pointer in an interrupt context
	 * (i.e., replenish timer).
	 */
	bool		has_backlogged_tasks;
...
#define dbg_cgx(cgx, str, ...) do {						\
	cbw_dbg(str "cgid%llu -- cgx:budget_remaining: %lld -- "		\
		...
		"cgx:runtime_total_at_replenish: %lld -- "			\
		"cgx:nquota: %lld -- "						\
		"cgx:nquota_ub: %lld -- "					\
		"cgx:is_throttled: %d -- cgx:nr_taskable_descendent: %d -- "	\
		"cgx:budget_p2c: %llu -- cgx:budget_c2l: %llu",			\
		##__VA_ARGS__,							\
		cgx->id, cgx->budget_remaining, cgx->runtime_total_sloppy,	\
		cgx->runtime_total_at_replenish,				\
		cgx->nquota, cgx->nquota_ub, cgx->is_throttled,			\
		cgx->nr_taskable_descendent, cgx->budget_p2c, cgx->budget_c2l);	\
} while (0);
...
s64 cbw_calc_budget_tx(struct scx_cgroup_ctx *cgx, s64 base_unit, int nr_branch)
{
	...
	tgt_unit = div_round_up(base_unit, nr_branch);

	return clamp(tgt_unit, CBW_BUDGET_XFER_MIN, cgx->nquota_ub);
}
			  ...
			  struct scx_cgroup_ctx *cgx)
{
	...
	base = subroot_cgx->budget_p2c;
	if (base != CBW_RUNTUME_INF) {
		nr_branch_cgs = cgx->nr_taskable_descendent +
				(cgx->has_llcx ? 1 : 0);
	...
	base = cgx->budget_p2c;
	if (base != CBW_RUNTUME_INF)
		cgx->budget_c2l = cbw_calc_budget_tx(cgx, base,
						     cbw_config.nr_llcs);
	else
...
static
int cbw_update_nr_taskable_descendent(struct cgroup *cgrp __arg_trusted,
				      int delta)
{
	 ...
	 * So nr_taskable_descendent matters only for subroot cgroups.
	 */
	subroot_cgrp = bpf_cgroup_ancestor(cgrp, 1);
	if (!subroot_cgrp) {
		cbw_err("Failed to lookup a subroot cgroup: %llu",
	 ...
	 * nr_taskable_descendent of a subroot. Since a budget transfer
	 * unit of a subroot cgroup is updated, all its descendants
	 * should be updated as well.
	 */
	subroot_cgx->nr_taskable_descendent += delta;
...
int scx_cgroup_bw_init(struct cgroup *cgrp __arg_trusted, struct scx_cgroup_init_args *args __arg_trusted)
{
	 ...
	 */
	cgx->nr_taskable_descendent = 1;
	return cbw_update_nr_taskable_descendent(cgrp, 1);
}
...
int scx_cgroup_bw_exit(struct cgroup *cgrp __arg_trusted)
{
	...
	if (cgrp->level > 1) {
		if ((ret = cbw_update_nr_taskable_descendent(cgrp, -1)))
			return ret;
	}
			    ...
			    struct scx_cgroup_llc_ctx *tgt_llcx)
{
	 ...
	 * to borrow budget.
	 */
	if ((src_level == 1) && (tgt_llcx->budget_remaining < 0))
 ...
 */
__hidden
int scx_cgroup_bw_put_aside(struct task_struct *p __arg_trusted, u64 ctx, u64 vtime, struct cgroup *cgrp __arg_trusted, int llc_id)
{
...
bool cbw_replenish_taskable_cgroup(struct scx_cgroup_ctx *cgx, int level, u64 now)
{
	...
	debt = cgx->runtime_total_at_replenish - cgx->nquota_ub;
	if (cgx->burst > 0) {
		if (debt < 0)
	 ...
	 * to its descendants.
	 *
	 * For non-subroot-level cgroups, we don't replenish the budget.
	 * instead, we limit the accumulation to the burst limit.
	 *
	 * If the cgroup has a debt (i.e., overused the budget beyond its upper
	 * bound), it should pay off the debt.
	 */
	...
	if (READ_ONCE(cgx->budget_remaining) > 0)
		WRITE_ONCE(cgx->budget_remaining, budget);
	else
		__sync_fetch_and_add(&cgx->budget_remaining, budget);
...
out_no_replenish:
	...
	if (cbw_has_backlogged_tasks(cgx))
		return true;
...
int replenish_timerfn(void *map, int *key, struct bpf_timer *timer)
{
	...
	/*
	 * Update the runtime total of each context to calculate
	 * the budget replenished.
	 */
	...
	bpf_for_each(css, pos, subroot_css, BPF_CGROUP_ITER_DESCENDANTS_POST) {
		...
		cur_cgx->runtime_total_at_replenish =
			cur_cgx->runtime_total_sloppy;
				  ...
				  struct scx_cgroup_llc_ctx *llcx)
{
	...
	if ((taskc == NULL) &&
	    READ_ONCE(llcx->has_backlogged_tasks) &&
	    !scx_atq_nr_queued(llcx->btq)) {
		WRITE_ONCE(llcx->has_backlogged_tasks, false);
	}
			 ...
			 struct scx_cgroup_ctx *cgx, u64 cgrp_id, u64 nuance)
{
	...
	bpf_for(i, 0, cbw_config.nr_llcs) {
		idx = (nuance + i) % cbw_config.nr_llcs;
		...
		scx_cgroup_bw_throttled(cgrp, idx);
...
int scx_cgroup_bw_reenqueue(void)
{
	...
	/*
	 * Unlock the reenqu lock so that others can start
