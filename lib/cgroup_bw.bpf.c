			    ...
			    struct scx_cgroup_llc_ctx *tgt_llcx)
{
	...
	do {
		...
		__sync_fetch_and_add(&tgt_llcx->budget_remaining_before, b);
	} while ((tgt_llcx->budget_remaining_before <= 0) &&
		 (src_cgx->budget_remaining > 0) && can_loop);

	return tgt_llcx->budget_remaining_before;
}
...
void cbw_reserve_budget(struct scx_cgroup_llc_ctx *llcx, u64 slice_ns)
{
	 ...
	 * budget_remaining_before, which saves cache coherence traffic.
	 */
	if (llcx->budget_remaining_before == CBW_RUNTUME_INF)
		return;

	__sync_fetch_and_sub(&llcx->budget_remaining_before, slice_ns);
}

static
struct cgroup *cbw_get_subroot_cgroup(struct cgroup *cgrp __arg_trusted)
{
	...
	bpf_rcu_read_lock();
	css = &cgrp->self;
	bpf_for_each(css, pos, css, BPF_CGROUP_ITER_ANCESTORS_UP) {
		cur_cgrp = pos->cgroup;
		if (cur_cgrp->level == 1) {
			bpf_rcu_read_unlock();
			return cur_cgrp;
 ...
 * @llc_id: LLC id where this function is called.
...
int scx_cgroup_bw_reserve(struct cgroup *cgrp __arg_trusted, int llc_id, u64 slice_ns)
{
	...
	if (READ_ONCE(llcx->budget_remaining_before) > 0) {
	 ...
	 */
	subroot_cgrp = cbw_get_subroot_cgroup(cgrp);
	if (subroot_cgrp == cgrp)
		return -EAGAIN;
