...
void BPF_STRUCT_OPS(lavd_enqueue, struct task_struct *p, u64 enq_flags)
{
	 ...
	 * but bounced -- typically because SCX_OPS_ALWAYS_ENQ_IMMED applied
	 * SCX_ENQ_IMMED to a local-DSQ insert and the CPU was not immediately
	 * available. The task has not run since, so its deadline, slice,
	 * CPU choice, cgroup-throttle state, and queued-load accounting are
	 * still valid -- reuse the cached suggested_cpu_id and reinsert into
	 * the previously chosen cpdom DSQ. Never the local DSQ it was just
	 * bounced from.
	 */
...
kick_cpu_out:
	/*
	 * Kick @cpu so an idle CPU picks up the task. SCX_KICK_IDLE is a
	 * no-op on non-idle CPUs, so issuing it unconditionally is safe;
	 * the explicit guards below just elide the kfunc call. Skip when:
	 *  1) ops.select_cpu() already designated this CPU for the enqueue;
	 *     the kernel will deliver the wake-up itself.
	 *  2) The task is currently running on @cpu (only possible for
	 *     SCX_TASK_REENQ_PREEMPTED-style reenqueues); the CPU already
	 *     has it dispatched.
	 */
	if (!__COMPAT_is_enq_cpu_selected(enq_flags) &&
	    !(cpu == task_cpu && scx_bpf_task_running(p)))
