		    ...
		    struct scx_cpu_release_args *args)
{
	 ...
	 * this slot out on newer kernels that either lift the restriction
	 * or already drain the local DSQ natively via
	     ...
	     unsigned int prev_state)
{
	...
	if (unlikely(next->prio < MAX_RT_PRIO &&
		...
		/*
		 * Reenqueue any SCX tasks stranded on the local DSQ. The
		 * compat wrapper resolves to the v2 (call-from-anywhere)
		 * kfunc on kernels that have it. This hook is autoloaded
		 * by Rust only on those kernels, so the verifier never
		 * sees a v1 call.
		 */
		scx_bpf_reenqueue_local();
	}
