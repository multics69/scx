...
static
s32 pick_idle_cpu_at_cpdom(struct pick_ctx *ctx, s64 cpdom, bool *is_idle)
{
	...
	if (!ctx->iat_empty && cpdc->is_active && cpdc->is_big) {
		...
		cpu = scx_bpf_pick_idle_cpu(cast_mask(ctx->temp_mask), 0);
		if (cpu >= 0) {
			*is_idle = true;
			return cpu;
		}
...
static __always_inline
bool can_run_on_domain(struct pick_ctx *ctx, struct task_struct *p, s64 cpdom)
{
	struct cpdom_ctx *cpdc;
...
static __always_inline
bool test_cpu_stickable(struct pick_ctx *ctx, struct task_struct *p,
...
s32 pick_idle_cpu(struct pick_ctx *ctx, bool *is_idle)
{
	 ...
	 * 
	 * 5) Minimize cross-domain migration: Before migrating a neighbor
	...
	if (!i_smt_empty) {
		...
		if (!ctx->ia_empty || !ctx->io_empty) {
			cpu = pick_idle_cpu_at_cpdom(ctx, sticky_cpdom, is_idle);
			if (cpu >= 0)
				goto unlock_out;
		}
	}
	 ...
	 * sticky domain.
	 */
	if (sticky_cpu >= 0 && (sticky_cpu == ctx->waker_cpu)) {
		cpu = sticky_cpu;
		goto unlock_out;
	}
	...
	for (i = 0; i < LAVD_CPDOM_MAX_DIST && cpdc; i++) {
		...
		if (nr_nbr == 0)
			break;
		for (j = 0; j < LAVD_CPDOM_MAX_NR; j++, nuance = q + 1) {
			if (j >= nr_nbr)
				break;
			q = pick_any_bit(cpdc->neighbor_bits[i], nuance);
			if (q == -ENOENT)
				continue;
			cpu = pick_idle_cpu_at_cpdom(ctx, q, is_idle);
			if (cpu >= 0)
				goto unlock_out;
		}
	}
