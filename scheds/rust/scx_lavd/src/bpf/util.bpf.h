...
/*
 * Bounce a ravg_data between the arena and the stack.
 *
 * lib/ravg.h's ravg_{to,from}_arena() copy whole u64 words with an unrolled
 * loop, so the field offsets fold into base+offset addressing on the incoming
 * pointer. Where that pointer is a task context the caller holds in a plain
 * register -- spilled and reloaded as a scalar -- clang drops the address-space
 * cast and the verifier rejects the access. Copying through an arena-qualified
 * pointer in a real loop recomputes the address every iteration, so the cast
 * survives.
 */
static __always_inline void
ravg_invr_from_arena(struct ravg_data *to, struct ravg_data __arena *from)
{
...
static __always_inline void
ravg_invr_to_arena(struct ravg_data __arena *to, struct ravg_data *from)
{
	for (int i = 0; i < sizeof(*to) && can_loop; i++)
