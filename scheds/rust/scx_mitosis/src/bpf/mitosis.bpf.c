			    ...
			    struct cgroup *cgrp)
{
	...
	adj_load(p, tctx, cgrp, p->scx.weight, scx_bpf_now_ns());
}
...
void BPF_STRUCT_OPS(mitosis_running, struct task_struct *p)
{
	...
	tctx->started_running_at = scx_bpf_now_ns();
}
...
void BPF_STRUCT_OPS(mitosis_stopping, struct task_struct *p, bool runnable)
{
	...
	used = scx_bpf_now_ns() - tctx->started_running_at;
...
static inline void quiescent(struct task_struct *p, struct cgroup *cgrp)
{
	...
	adj_load(p, tctx, cgrp, -(s64)p->scx.weight, scx_bpf_now_ns());
}
