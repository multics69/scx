...
struct scx_cgroup_llc_ctx {
	 ...
	 * When @budget_remaining_before becomes zero (or negative), we ask
	 ...
	 */
	s64		budget_remaining_before;

	/*
	 * The amount of remaining time out of @quota after task execution.
	 * It can be negative when the quota is over-used. When
	 * @budget_remaining_before is replenished, it will be updated
	 * accordingly too.
	 */
	s64		budget_remaining_after;
...
#define REGISTER_SCX_CGROUP_BW_ENQUEUE_CB(enqueue_cb)				\
	int scx_group_bw_enqueue_cb(pid_t pid)					\
	{									\
		extern void enqueue_cb(pid_t pid);				\
		enqueue_cb(pid);						\
		return 0;							\
	}
